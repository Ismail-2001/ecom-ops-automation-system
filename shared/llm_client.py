...
class CircuitBreaker:
    ...
    def is_open(self) -> bool:
        ...
        if self._failures >= self.threshold:
            ...
            logger.warning("circuit_breaker=open failures=%d recovery_timeout=%.0fs", self._failures, self.recovery_timeout)
            return True
        return False

    def success(self):
        if self._failures > 0:
            logger.info("circuit_breaker=closed failures=%d", self._failures)
        self._failures = 0
        self._open_until = 0.0
    ...
    def failure(self):
        self._failures += 1
        if self._failures >= self.threshold:
            logger.warning("circuit_breaker=tripped failures=%d threshold=%d", self._failures, self.threshold)
...
class LLMClient:
    def __init__(
        ...
        logger.info("llm_client=init model=%s provider=%s max_retries=%d", self.model, "gemini" if self._use_gemini else "openai", self.max_retries)
    ...
    async def call(self, user_prompt: str, response_model: Optional[Type[T]] = None) -> LLMResult:
        if self.circuit_breaker.is_open:
            logger.warning("llm_call=blocked reason=circuit_breaker_open")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.info("llm_call=success model=%s attempt=%d latency_ms=%.0f", self.model, attempt + 1, latency)
                return result
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.warning("llm_call=retry attempt=%d error=%s wait=%.1fs", attempt + 1, type(e).__name__, wait)
                    await asyncio.sleep(wait)

        logger.error("llm_call=failed attempts=%d error=%s", self.max_retries, type(last_error).__name__)
    ...
    async def _do_call(self, prompt: str) -> str:
        if not self._api_key:
            logger.info("llm_call=mock no_api_key_configured")
            return ""
