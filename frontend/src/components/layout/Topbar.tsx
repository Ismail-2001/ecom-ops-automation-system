...
export default function Topbar({ title, subtitle, actions, onMenuToggle }: {
  ...
  return (
    <header className="sticky top-0 z-40 h-14 sm:h-16 bg-void/80 backdrop-blur-xl border-b border-border flex items-center px-4 sm:px-6 gap-3">
      <button
        onClick={onMenuToggle}
        className="lg:hidden w-9 h-9 rounded-button flex items-center justify-center text-text-muted hover:bg-surface-2 hover:text-text-primary transition-colors"
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        <div className="relative hidden md:flex items-center">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-56 lg:w-72 h-9 pl-9 pr-4 rounded-button bg-surface border border-border text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary/30 transition-colors"
          />
        ...
        <div className="hidden sm:block w-9 h-9 rounded-button flex items-center justify-center text-text-muted hover:bg-surface-2 hover:text-text-primary transition-colors">
